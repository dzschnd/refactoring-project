...
const mockGetInvitationQuery = jest.fn();
const mockGetAllFormQuestionsQuery = jest.fn();
const mockDeleteGuestAnswersQuery = jest.fn();
const mockCreateGuestAnswerQuery = jest.fn();
const mockGetAllGuestAnswersQuery = jest.fn();
...
describe("PublishedInvitationService.submitGuestAnswers", () => {
    it("submits guest answers successfully", async () => {
        ...
        const result = await submitGuestAnswers(1, [
    ...
    it("returns error when not all questions are answered", async () => {
        ...
        const result = await submitGuestAnswers(1, [{ questionPosition: 0, answer: "A1" }], "Guest", true, "guest-1");
        expect("error" in result).toBe(true);
        if ("error" in result) {
