...
export const getAllInvitations = async (req, res) => {
    ...
    if (allInvitationsInfo.error) {
        return res.status(500).json(errorResponse(invitationInfo.status === 500 ? SERVER_ERROR : invitationInfo.error));
    }
    res.status(200).json(allInvitationsInfo);
};
...
export const submitGuestAnswers = async (req, res) => {
    ...
    const guestAnswer = await PublishedInvitationService.submitGuestAnswers(
        invitationId,
        answers,
        guestName,
        isComing,
    ...
    res.status(201).json(guestAnswer);
};
...
export const getAllGuestAnswers = async (req, res) => {
    ...
    res.status(200).json(allGuestAnswers);
};
