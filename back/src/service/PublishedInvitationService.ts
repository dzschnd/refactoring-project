...
import {
    ...
    getAllGuestAnswersQuery,
    deleteGuestAnswersQuery,
    getAllFormQuestionsQuery
...
export const getInvitation = async (invitationId: number | string): Promise<ServiceResponse<InvitationDetailsDTO>> => {
    try {
...
export const getAllInvitations = async (userId: number): Promise<ServiceResponse<InvitationDetailsDTO[]>> => {
    try {
        ...
        return details.filter((invitation): invitation is NonNullable<typeof invitation> => invitation !== null);
    } catch (error) {
...
export const getAllGuestAnswers = async (authorId: number): Promise<ServiceResponse<GuestAnswer[]>> => {
    try {
