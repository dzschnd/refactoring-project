...
export const getInvitation = async (
  ...
  try {
    ...
    if (!invitationInfo) return errorResponse("Invitation not found");
...
export const getAllInvitations = async (
  ...
  try {
    ...
    const details = await Promise.all(
      existingInvitations.map((invitation) => getInvitationDetails(invitation.id, true)),
    );
...
export const submitGuestAnswers = async (
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      const answeredQuestionsPositions = new Set(
        answers
          .sort((a, b) => a.questionPosition - b.questionPosition)
          .map((answer) => answer.questionPosition),
      );
      const existingQuestionsPositions = new Set(
        existingQuestions
      ...
      for (const answer of answers) {
        const answeredQuestion = existingQuestions.filter(
          (question) => question.position === answer.questionPosition,
        );
        if (answeredQuestion.length === 0) {
          return errorResponse("Question not found");
        }

        await createGuestAnswerQuery(
          invitationId,
...
export const getAllGuestAnswers = async (
  authorId: number,
): Promise<ServiceResponse<GuestAnswer[]>> => {
  try {
