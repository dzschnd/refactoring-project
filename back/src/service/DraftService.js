...
export const createDraft = async (userId, templateName) => {
    try {
        return await prisma.$transaction(async (tx) => {
            ...
            return await getInvitationDetails(draft[0].id, false, tx);
        });
    } catch (error) {
...
export const updateDraft = async (draftId, data, userId) => {
    try {
        return await prisma.$transaction(async (tx) => {
            ...
            return await getInvitationDetails(draftId, false, tx);
        });
    } catch (error) {
        console.log(error);
...
export const publishDraft = async (invitationId, userId) => {
    try {
        return await prisma.$transaction(async (tx) => {
            ...
            return await getInvitationDetails(invitationId, true, tx);
        });
    } catch (error) {
...
export const getAllDrafts = async (userId) => {
    try {
        ...
        return await Promise.all(drafts.map(draft => getInvitationDetails(draft.id, false)));
    } catch (error) {
        console.log(error)
