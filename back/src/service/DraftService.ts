...
export const uploadImageToDraft = async (
  ...
  if (!(type === "coupleImage" || type === "placeImage")) return errorResponse("Invalid type");
  ...
  const updateResult = await updateDraft(
    ...
    type === "coupleImage"
      ...
      : { place: { ...safePlace, placeImage: imageUrl } },
    userId,
  );
...
export const resetDraftImage = async (
  draftId: number | string,
  type: string,
  userId: number,
): Promise<ServiceResponse<InvitationDetailsDTO>> => {
  ...
  const updateResult = await updateDraft(
    ...
    type === "coupleImage"
      ? { coupleImage: null }
      : { place: { address: null, placeImage: null, link: null } },
    userId,
  );
  if ("error" in updateResult) {
    return errorResponse("Failed to upload image");
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (place) {
        ...
        if (placeRecord.length && placeRecord[0].placeId) {
          await updatePlaceQuery(
            place.address,
            place.placeImage,
            place.link,
            placeRecord[0].placeId,
            tx,
          );
        } else {
      ...
      if (questions) {
        ...
        for (const question of questions) {
          const newQuestion = await createFormQuestionQuery(
            question.question,
            question.type,
            question.position,
            draftId,
      ...
      if (answers) {
        ...
        for (const answer of answers) {
          const question = await getFormQuestionByPositionQuery(
            draftId,
            answer.questionPosition,
            tx,
          );
          if (
            questionsList.filter((q) => q.position === answer.questionPosition).length === 0 &&
            question.length === 0
          ) {
...
export const publishDraft = async (
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      if (existingDraft.length === 0) return errorResponse("Draft not found");
...
export const getDraft = async (
  ...
  try {
    ...
    if (!draftInfo || draftInfo.authorId !== userId) return errorResponse("Draft not found");
    return draftInfo;
  } catch (error) {
...
export const deleteDraft = async (
  ...
  try {
    ...
    if (draft.length === 0) return errorResponse("Draft not found");
