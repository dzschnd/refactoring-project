...
export const uploadImageToDraft = async (draftId: number | string, file: Express.Multer.File, type: string, userId: number): Promise<ServiceResponse<InvitationDetailsDTO>> => {
    let draft;
    let currentPlace;
    ...
    const updateResult = await updateDraft(
        draftId,
        type === "coupleImage" ? {coupleImage: imageUrl} : {place: {...safePlace, placeImage: imageUrl}},
        userId);
...
export const resetDraftImage = async (draftId: number | string, type: string, userId: number): Promise<ServiceResponse<InvitationDetailsDTO>> => {
    if (!(type === "coupleImage" || type === "placeImage"))
        return errorResponse("Invalid type");

    const updateResult = await updateDraft(
        draftId,
        type === "coupleImage" ? {coupleImage: null} : {place: {address: null, placeImage: null, link: null}},
        userId);
    if ("error" in updateResult) {
        return errorResponse("Failed to upload image");
    } else {
...
export const updateDraft = async (draftId: number | string, data: DraftUpdateDTO, userId: number): Promise<ServiceResponse<InvitationDetailsDTO>> => {
    try {
        return await prisma.$transaction(async (tx) => {
            ...
            const insertedQuestions: Array<{ id: number }> = [];
            if (questions) {
...
export const publishDraft = async (invitationId: number | string, userId: number): Promise<ServiceResponse<InvitationDetailsDTO>> => {
    try {
        return await prisma.$transaction(async (tx) => {
...
export const getDraft = async (draftId: number | string, userId: number): Promise<ServiceResponse<InvitationDetailsDTO>> => {
    try {
...
export const getAllDrafts = async (userId: number): Promise<ServiceResponse<InvitationDetailsDTO[]>> => {
    try {
...
export const deleteDraft = async (draftId: number | string, userId: number): Promise<ServiceResponse<{ message: string }>> => {
    try {
