...
export const refreshTokens = async (
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      if (existingTokens.length === 0) return errorResponse("Invalid refresh token");
...
export const requestResetPassword = async (
  email: string,
): Promise<ServiceResponse<{ message: string; user: User }>> => {
  try {
    return await prisma.$transaction(async (tx) => {
      const user = await getUserByEmailQuery(email, tx);
...
export const verifyEmail = async (email: string, otp: string): Promise<ServiceResponse<User>> => {
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      if (existingUsers.length === 0) return errorResponse("User not found");

      const existingOtp = await getOtpQuery(otp, existingUsers[0].id, tx);

      if (existingOtp.length === 0) return errorResponse("OTP not found");

      if (new Date() > new Date(existingOtp[0].expiresAt)) return errorResponse("OTP has expired");

      if (!existingUsers[0].verified) await activateUserQuery(existingUsers[0].id, tx);
...
export const checkPassword = async (
  ...
  try {
    ...
    if (!isPasswordValid) return errorResponse("Invalid credentials");
...
export const login = async (
  ...
  ServiceResponse<{ user: Omit<User, "password">; accessToken: string; refreshToken: string }>
> => {
  try {
    return await prisma.$transaction(async (tx) => {
      const existingUser = await getUserByEmailQuery(email, tx);

      if (existingUser.length === 0) return errorResponse("Invalid credentials");

      const checkPasswordResponse = await checkPassword(existingUser[0], password, tx);

      if ("error" in checkPasswordResponse) return errorResponse("Invalid credentials");
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      if (newEmail) {
        ...
        if (existingUsersByEmail.length > 0) return errorResponse("Email is not available");
