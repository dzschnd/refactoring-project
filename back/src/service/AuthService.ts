...
type OtpResponse = { message: string };
type ProfileResponse = { message: string; newEmail?: string | null; newName?: string | null };
...
export const createAndSendOtp = async (userId: number, email: string, client: TxClient | PrismaClient = prisma): Promise<ServiceResponse<OtpResponse>> => {
...
export const requestOtp = async (email: string): Promise<ServiceResponse<OtpResponse>> => {
    try {
...
export const register = async (email: string, password: string): Promise<ServiceResponse<Omit<User, "password">>> => {
    try {
        return await prisma.$transaction(async (tx) => {
            const user = await getUserByEmailQuery(email, tx);
...
export const refreshTokens = async (refreshToken: string, user: { id: number; email: string }): Promise<ServiceResponse<Tokens>> => {
    try {
        return await prisma.$transaction(async (tx) => {
...
export const requestResetPassword = async (email: string): Promise<ServiceResponse<{ message: string; user: User }>> => {
    try {
        return await prisma.$transaction(async (tx) => {
            const user = await getUserByEmailQuery(email, tx);
...
export const requestChangeEmail = async (email: string, newEmail: string): Promise<ServiceResponse<OtpResponse>> => {
    try {
        return await prisma.$transaction(async (tx) => {
            const user = await getUserByEmailQuery(email, tx);
            if (user.length === 0) return errorResponse("User not found");
...
export const verifyEmail = async (email: string, otp: string): Promise<ServiceResponse<User>> => {
    try {
        return await prisma.$transaction(async (tx) => {
...
export const activateUser = async (user: { id: number; email: string }): Promise<ServiceResponse<Tokens>> => {
    try {
        return await prisma.$transaction(async (tx) => {
...
export const checkPassword = async (user: Pick<User, "email" | "password">, password: string, client: TxClient | PrismaClient = prisma): Promise<ServiceResponse<{ message: string }>> => {
    try {
        let isPasswordValid;
        if (!user.password) {
...
export const login = async (email: string, password: string): Promise<ServiceResponse<{ user: Omit<User, "password">; accessToken: string; refreshToken: string }>> => {
    try {
        return await prisma.$transaction(async (tx) => {
...
export const getProfile = async (user: { id: number }): Promise<ServiceResponse<Omit<User, "password">>> => {
    try {
...
export const logout = async (user: { id: number }): Promise<ServiceResponse<{ message: string }>> => {
    try {
        const currentTime = getCurrentTime();
...
export const updateProfile = async (newEmail: string | null, newPassword: string | null, email: string): Promise<ServiceResponse<ProfileResponse>> => {
    try {
        return await prisma.$transaction(async (tx) => {
            const existingUsers = await getUserByEmailQuery(email, tx);
...
export const changeEmail = async (oldEmail: string, newEmail: string): Promise<ServiceResponse<ProfileResponse>> => {
    try {
        return await prisma.$transaction(async (tx) => {
...
export const changePassword = async (newPassword: string, email: string): Promise<ServiceResponse<{ message: string }>> => {
    try {
        return await prisma.$transaction(async (tx) => {
            const existingUsers = await getUserByEmailQuery(email, tx);
...
export const changeName = async (newName: string, email: string): Promise<ServiceResponse<ProfileResponse>> => {
    try {
        return await prisma.$transaction(async (tx) => {
            const existingUsers = await getUserByEmailQuery(email, tx);
